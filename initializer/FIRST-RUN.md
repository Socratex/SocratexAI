...
17. Project Profile Interview:
   ...
   - stack tags, suggested from `tools/setup/detect_project_stack.py` when practical
18. Run `tools/quality/check_runtime.py` when practical and record runtime availability.
   - If Python 3.10+ is missing, present a Python install or repair plan.
   - Ask for explicit user approval before installing or changing system packages.
   - If Python tooling is unsupported, recommend lite/no-tools mode, a supported host/container, or porting required scripts.
...
13. Compile project-local agent instructions with `tools/pipeline/generate_installed_agent_instructions.py` when practical.
14. Run `tools/documents/audit_docs.py -Initialized` for initialized projects when practical.
