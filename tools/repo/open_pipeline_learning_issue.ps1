...
function ConvertTo-IssueBody {
	...
	foreach ($line in @(
		...
		"",
		"- ``tools/repo/report_pipeline_learning.ps1`` compares a project ``pipeline_featurelist.json`` with the source manifest and classifies unknown feature IDs.",
		"- ``tools/repo/open_pipeline_learning_issue.ps1`` turns that report into this prefilled GitHub Issue URL without using a write token or API write.",
		"- ``tools/repo/learn_pipeline_features.ps1`` is the maintainer-side promotion tool for reviewed reusable feature IDs.",
		"- ``tools/repo/sync_pipeline_featurelist.ps1`` propagates source feature IDs back into installed project instance manifests after the source learns something reusable.",
		"",
		...
		"",
		'```powershell',
		'powershell -NoProfile -ExecutionPolicy Bypass -File tools/repo/learn_pipeline_features.ps1 -ProjectPath "<project>" -Apply -IncludeFeatures <ids>',
		'```',
		"",
		"4. After promotion, update or reinitialize consuming projects so ``tools/repo/sync_pipeline_featurelist.ps1`` can refresh their instance manifests.",
		"",
		"## Reporter Notes",
		""
	)) {
