...
if (-not (Test-Path -LiteralPath $gatePath -PathType Leaf)) {
	throw "Changed code paths require a fresh full compiled code-guidance load. Run tools/knowledge/knowledge_code_context.ps1 before code work."
}

$gate = Get-Content -LiteralPath $gatePath -Raw | ConvertFrom-Json
if (-not $gate.full_base_loaded) {
	throw "Code context gate marker does not confirm a full base code-guidance load. Run tools/knowledge/knowledge_code_context.ps1."
}
...
if ($MaxAgeMinutes -gt 0) {
	...
	if ($ageMinutes -gt $MaxAgeMinutes) {
		throw "Code context gate marker is stale ($([math]::Round($ageMinutes, 1)) minutes old). Run tools/knowledge/knowledge_code_context.ps1 again."
	}
}

$currentHead = (git -C $repoRoot rev-parse HEAD).Trim()
if ([string]$gate.repo_head -ne $currentHead) {
	throw "Code context gate marker was loaded for a different HEAD. Run tools/knowledge/knowledge_code_context.ps1 again."
}

$selectedTags = @($gate.selected_tags | ForEach-Object { [string]$_ })
$missingTags = @($requiredTags | Where-Object { $_ -notin $selectedTags })
if ($missingTags.Count -gt 0) {
	throw "Code context gate marker is missing required tags: $($missingTags -join ', '). Run tools/knowledge/knowledge_code_context.ps1."
}
