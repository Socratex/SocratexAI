...
function Get-ScriptCatalog {
    if ($null -ne $script:scriptCatalog) {
        return $script:scriptCatalog
    }

    $catalogPath = Join-Path $repoRoot "SCRIPTS.json"
    if (-not (Test-Path -LiteralPath $catalogPath)) {
        Add-Error "Missing required file: SCRIPTS.json"
        return $null
    }

    $script:scriptCatalog = Get-Content -Raw -LiteralPath $catalogPath | ConvertFrom-Json
    return $script:scriptCatalog
}
...
function Test-ScriptExists {
    ...
    $relativePath = $catalog.content.$Name.path
    if ([string]::IsNullOrWhiteSpace($relativePath)) {
        Add-Error "Script catalog entry has no path: $Name"
        return
    }

    if (-not (Test-Path -LiteralPath (Join-Path $repoRoot $relativePath))) {
        Add-Error "Missing script file from catalog: $relativePath"
    }
}
...
try {
    ...
    $evalCheckScript = Join-Path $repoRoot "tools/quality/check_evals.ps1"
    if (Test-Path -LiteralPath $evalCheckScript) {
        ...
        Add-Error "Missing eval framework checker: tools/quality/check_evals.ps1"
    }
    $featureContractCheckScript = Join-Path $repoRoot "tools/repo/check_pipeline_feature_contracts.ps1"
    if (Test-Path -LiteralPath $featureContractCheckScript) {
        ...
        if ($LASTEXITCODE -ne 0) {
            Add-Error "Pipeline feature contract check failed with exit code $LASTEXITCODE"
        }
    } else {
        Add-Error "Missing pipeline feature contract checker: tools/repo/check_pipeline_feature_contracts.ps1"
    }
    ...
    Test-ContainsText -Text $contract -Needle "tools/documents/review_document_list_candidates.ps1" -Label "core/AGENT-CONTRACT.json"
    ...
    if ($codePackActive) {
        ...
        Test-ContainsText -Text $codeDiagnostics -Needle "tools/diagnostics/log_summary.ps1" -Label "project/code/DIAGNOSTICS.json"
        ...
        Test-ContainsText -Text (Get-RepoText -RelativePath "tools/pipeline/set_directives.ps1") -Needle ".old" -Label "tools/pipeline/set_directives.ps1"
        Test-ContainsText -Text (Get-RepoText -RelativePath "tools/pipeline/set_directives.ps1") -Needle "SOCRATEX.md" -Label "tools/pipeline/set_directives.ps1"
        Test-ContainsText -Text (Get-RepoText -RelativePath "tools/pipeline/Initialize-SocratexPipeline.ps1") -Needle "WORKFLOW.json" -Label "tools/pipeline/Initialize-SocratexPipeline.ps1"
        Test-ContainsText -Text (Get-RepoText -RelativePath "tools/pipeline/Initialize-SocratexPipeline.ps1") -Needle "team\pipeline.json" -Label "tools/pipeline/Initialize-SocratexPipeline.ps1"
        Test-ContainsText -Text (Get-RepoText -RelativePath "tools/pipeline/import_existing_project.ps1") -Needle "WORKFLOW.json" -Label "tools/pipeline/import_existing_project.ps1"
        Test-ContainsText -Text (Get-RepoText -RelativePath "tools/pipeline/import_existing_project.ps1") -Needle "team/pipeline.json" -Label "tools/pipeline/import_existing_project.ps1"
        Test-ContainsText -Text (Get-RepoText -RelativePath "tools/pipeline/reinitialize_pipeline.ps1") -Needle "WORKFLOW.json" -Label "tools/pipeline/reinitialize_pipeline.ps1"
        Test-ContainsText -Text (Get-RepoText -RelativePath "tools/pipeline/reinitialize_pipeline.ps1") -Needle "team/pipeline.json" -Label "tools/pipeline/reinitialize_pipeline.ps1"
    }
