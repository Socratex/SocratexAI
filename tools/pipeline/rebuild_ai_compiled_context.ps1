...
$knowledgeCompileScript = Join-Path $repoRoot "tools\knowledge\knowledge_compile.ps1"
$knowledgeCheckScript = Join-Path $repoRoot "tools\knowledge\knowledge_check.ps1"
...
function Get-Section {
	...
	$script = Join-Path $repoRoot "tools\documents\read_document_item.ps1"
...
"@
	...
	$index = [ordered]@{
		...
		recompile_command = "powershell -NoProfile -ExecutionPolicy Bypass -File tools/pipeline/rebuild_ai_compiled_context.ps1"
		check_command = "powershell -NoProfile -ExecutionPolicy Bypass -File tools/pipeline/check_ai_compiled_context.ps1"
...
if ($Check) {
	...
	if ($drift.Count -gt 0) {
		...
		Write-Host "Run: powershell -NoProfile -ExecutionPolicy Bypass -File tools/pipeline/rebuild_ai_compiled_context.ps1"
		exit 1
	}
