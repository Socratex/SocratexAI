...
function Set-TemplateValue {
    ...
    Write-Utf8File -Path $destination -Value $content -NoNewline
}
...
function Ensure-BranchGitignore {
    ...
    if (-not (Test-Path -LiteralPath $gitignorePath)) {
        Write-Utf8File -Path $gitignorePath -Value "$comment`n$ignoredLine`n" -NoNewline
        return
    }
    ...
    if ($changed) {
        Write-Utf8File -Path $gitignorePath -Value $content -NoNewline
    }
}
...
function Initialize-AssistantLayout {
    ...
    Write-Utf8File -Path (Join-Path $assistantRoot "DOCS.md") -Value @"
...
"@ -NoNewline
    ...
    Write-Utf8File -Path (Join-Path $Root ".aiassistant\$projectFileName.md") -Value @"
# Project Rules
...
if ($CreateFiles) {
    ...
    if (-not $DryRun -and (Test-Path -LiteralPath (Join-Path $Root "pipeline_featurelist.json"))) {
        ...
        $syncFeatureListScript = Join-Path $Root "tools\repo\sync_pipeline_featurelist.ps1"
        ...
        $knowledgeCompileScript = Join-Path $Root "tools\knowledge\knowledge_compile.ps1"
...
if ($DryRun) {
    ...
    Write-Utf8File -Path $readmePath -Value $readme -NoNewline
}
...
if ($RunAudit -and $keep.ContainsKey("code")) {
    if ($DryRun) {
        ...
        & (Join-Path $Root "tools\documents\audit_docs.ps1") -Initialized
    }
}
