...
& powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PipelineRoot "tools\pipeline\import_existing_project.ps1") -TargetPath $TargetRoot -Packs $Packs -CreateProjectFiles:$CreateProjectFiles
& powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $TargetRoot "SocratexAI\tools\pipeline\set_directives.ps1") -TargetPath $TargetRoot -Mode $DirectiveMode -DirectiveFiles $existingDirectives

Write-Host "Migration complete. SocratexAI is now active for this project; future sessions should start from SOCRATEX.md."
Write-Host "Review SOCRATEX.md, any *.old directive files, and run SocratexAI/tools/documents/audit_docs.ps1."
